    ...
    for root, _, fnames in sorted(os.walk(dir)):
        for fname in fnames:
            if is_image_file(fname):
                ...
                path_names[4] = path_names[4].replace('_', '')
                path_names[5] = path_names[5].split('_')[0] + "_" + "".join(path_names[5].split('_')[1:])
                path_names = "".join(path_names)
                #new_path = os.path.join(dir, path_names)
                new_path = path_names[1:]
                img = Image.open(path)
